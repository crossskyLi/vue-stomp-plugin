...
class socketCaller {
  ...
  makeKeys(option) {
    const { topicKey, idKey, tokenKey, destinationKey, unsubdestinationKey } = option;
  ...
  onmessage() {
    this.client.ws.onmessage = (event) => {
      ...
      if (this.subscribeTopicMap[topic]) {
        this.subscribeTopicMap[topic].forEach(m => { m.callback(responseData) })
      }
    }
  }
  ...
  subscribe(topic, vm, callback, hookName = "$destroy", requestBody = {}) {
    ...
    this.subscribeTopicMap[topic].push({ instance: vm, [idKey]: vm[idKey], callback });
